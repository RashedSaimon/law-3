...
const handleHome = (_req: Request, res: Response) => {
  ...
  const articles = db.articles.filter(a => a.status === 'published').slice(0, 3);
  const hero_slides = db.heroSlides.filter(h => h.is_active).sort((a, b) => a.sort_order - b.sort_order);
...
const handleAdminContentPost = async (req: Request, res: Response) => {
  ...
  if (type === 'settings') {
    ...
    for (const [key, value] of Object.entries(newSettings)) {
      await db.setSetting(key, (value as string).trim());
    }
  ...
  if (action === 'save') {
    ...
    if (req.file) {
      data.image_path = '/uploads/' + req.file.filename;
    }
...
app.post('/admin/content', requireAdmin, upload.single('image'), handleAdminContentPost);
app.post('/admin/content.php', requireAdmin, upload.single('image'), handleAdminContentPost);
